# MapPublisher 再構成・地図データ公開移行 実装計画
...
| **2** | MapPublisher 再構成 | MapPublisher | — | **完了**（先行実施） |
| **1** | 公開API の実装。`api/_lib/` 共通化、`api/mapdata.js`・`api/manifest.js` 新規、`api/closures.js` 改修 | minoh-hiking | 0 | 未着手 |
| **3** | mapdata の初回公開 | MapEditor → MapPublisher | 1, 2 | 未着手 |
| **4** | minoh-hiking アプリ側の切替 | minoh-hiking | 3 | 未着手 |
| **5** | 後始末（バンドル geojson 削除・旧環境変数削除・ドキュメント改訂） | 全体 | 4 | 未着手 |

> Phase 2 を Phase 1 より先に実施した。API が未実装のため、MapPublisher 側は
> `/api/mapdata`・`/api/manifest` に対する**疎通確認が未了**である。
> Phase 1 の完了後、地図データの「現在公開中」表示と公開を実機で確認すること。

---

## 8. 移行手順（順序厳守）

1. **Vercel に `MAP_PUBLISH_TOKEN` を設定する**
   - コードのデプロイより**先**に行う。未設定のままデプロイすると 503（E02）になる
2. Phase 1 のコードをデプロイする
   - この時点で `GET /api/mapdata` は空の FeatureCollection を返す
3. MapEditor で統合geojson を出力する（**データ修正は不要**。公開スキーマへの整形は MapPublisher が行う）
4. MapPublisher で **mapdata を初回公開**する → `2026.1`
5. `GET /api/mapdata` を直接開き、668件・type構成・公開スキーマ・座標が正しいことを確認する
6. **確認できてから** Phase 4（アプリ側切替）をデプロイする
   - 順序を誤ると、全ユーザーの地図からルート・スポットが消える
7. 1リリース分の様子を見てから `public/data/*.geojson` を削除する（ロールバック余地を残す）
8. `CLOSURES_PUBLISH_TOKEN` を Vercel から削除する
9. Blob 上の旧履歴 `closures/history/` を `del()` で一括削除する（前回分1世代の方式に移行済みのため）

**運用者への影響**: localStorage キーを `closure-editor.publish-token` から `map-publisher.publish-token` へ変更するため、切替後に**公開トークンの再入力が1回だけ**発生する。
...
| MapPublisher | `docs/usersGuide-202608.md` | **改訂済み**（v1.1）。編集手順を削除し、地図データの公開手順を追加 |
