...
版: 1.8 / 作成日: 2026-08-18 / 最終更新: 2026-08-23
...
> **本書の守備範囲は契約 2.0 までの移行である。**
> ClosureEditor の再構成と、ハイキングマップデータを公開経由の配信へ移す作業を記録している。
> **これより後の変更は本書に反映しない。** 現在の仕様は次の3文書が正本である。
>
> | 変更 | 契約 | 正本 |
> |---|---|---|
> | `tiles`（地図タイルのダウンロード領域）の追加 | 2.1 | [機能仕様 §3.5](funcspec-202608.md) / [データ仕様 §5](dataspec-202608.md) |
> | 画面を開閉式に変更、出力を公開済みデータへ変更、失敗時の復元 | 2.1 | [機能仕様 §2.2・§3.7.4・§3.8](funcspec-202608.md) |
> | **`version` の採番を送信側へ移し、形式を `yyyy.nn` に統一**（§0-3 の決定を覆す） | **3.0** | [機能仕様 §3.7.1](funcspec-202608.md) / minoh-hiking `publish-api-202608.md` §4 |
...
| 2 | データセット数 | **2つ**。`mapdata`（緊急ポイント＋ルート＋スポット）/ `closures`（通行止め・通行困難）<br>※ 契約 2.1 で `tiles` を追加し3つになった（本計画の対象外） |
| 3 | version 採番 | **サーバー側で自動採番**。`mapdata` は `yyyy.n`、`closures` は `yyyy-mm.n`<br>※ 契約 3.0 で**送信側の指定**に変更し、形式も `yyyy.nn` に統一した（本計画の対象外） |
| 4 | n の桁 | **1桁**（ゼロ埋めしない） |
...
> **これは移行当時（契約 2.0）の画面である。** 現在は3セクションの開閉式で、
> 「現在公開中」は見出しにあり、出力ボタンは「公開済みデータを出力」になっている。
> 現行の画面構成は [機能仕様 §2](funcspec-202608.md) を参照すること。
...
|---|---|---|
| minoh-hiking | `docs/publish-api-202608.md` | **作成済み**。契約2.0 と公開スキーマの**正本**（その後 2.1 で `tiles` を追加） |
| MapPublisher | `docs/funcspec-202608.md` | **改訂済み**（本移行では v1.2。現在 **v1.6**）。ClosureEditor 名の解消、編集機能の削除、2データセット公開。§6 は `publish-api-202608.md` への依存項目のみに限定 |
| MapPublisher | `docs/dataspec-202608.md` | **改訂済み**（本移行では v2.2。現在 **v2.5**）。公開スキーマは正本を参照し、MapEditor から受け取る**入力ファイル**の仕様と整形規則に絞った |
| MapPublisher | `docs/usersGuide-202608.md` | **改訂済み**（本移行では v1.1。現在 **v1.5**）。編集手順を削除し、ハイキングマップデータの公開手順を追加 |
...
| 版 | 日付 | 内容 |
|---|---|---|
| 1.8 | 2026-08-23 | 契約 3.0（`version` を送信側が指定・形式 `yyyy.nn`）を冒頭の対象外一覧と §0-3 に追記 |
| 1.7 | 2026-08-23 | 本書の守備範囲が**契約 2.0 まで**であることを冒頭に明記し、以降の変更（契約 2.1 の `tiles` 追加・画面の開閉化・出力の変更）の正本を示した。§10 のドキュメント版数を現況に更新。ヘッダの版表記が 1.0 のまま変更履歴と食い違っていたのを修正 |
