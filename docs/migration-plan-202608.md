...
### 9.0 配信データが整形済みで、サイズが2.3倍になっている（要判断）

公開API は Blob へ `JSON.stringify(data, null, 2)`（インデント付き）で保存し、
GET はそれをそのまま返している（契約1.0 からの踏襲）。

| | サイズ |
|---|---|
| 実際の配信（インデント付き） | 478,894 B |
| 最小化した場合 | 208,247 B |

version ゲートで「変わったときだけ取得する」設計にした狙いからすると、
**取得1回あたり 270KB 余計に流れている**。弱電波下での更新を考えると削減の価値がある。

選択肢:

- **(A) 保存は整形済みのまま、配信時だけ最小化する** — Blob を人が直接読めるまま維持できる。**推奨**
- (B) 保存も最小化する — 単純だが、Blob の中身を目視で確認しにくくなる
- (C) 現状維持 — 更新頻度が低く、実害が小さいと判断する場合
